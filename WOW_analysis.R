...
dailywts <- dailywts_all[dailywts_all$dailywt > 0, ]

colnames(staticwts_all)[colnames(staticwts_all) == "Weight"] <- "staticwt"
staticwts_raw <- staticwts_all[staticwts_all$staticwt > 0, ]

# selecting animals for which static weight (reference weight) is available for further analysis
dailywts_raw <- dailywts %>%
  filter(RFID %in% unique(staticwts_raw$RFID))
  
# tally to get animals with static and daily wt records
staticwts <- staticwts_raw %>%
  filter(RFID %in% unique(dailywts_raw$RFID))
...
dailywts_herd1_avg <- dailywts_herd1_avg %>%
  group_by(RFID) %>%
...
# linear interpolation to get the wows for missing days
dailywts_herd2_avg <- dailywts_herd2_avg %>%
  group_by(RFID) %>%
  filter(n_distinct(Date) >= 2)
